...
// Fallback questions used when LLM API is unavailable
...
function LlmQuestions() {
  ...
  // Generate questions using real Gemini API, with fallback to local questions
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.questions) {
          // Handle real API response format - extract question_text from objects
          const formattedQuestions = data.questions.map(q => {
            // If it's already a string (fallback), use it directly
            if (typeof q === 'string') {
              return q;
            }
            // If it's an object from real API, extract question_text
            return q.question_text || q.question || 'Question text not available';
          });
          setGeneratedQuestions(formattedQuestions);
          
          // Determine source based on first question
          if (data.questions.length > 0 && typeof data.questions[0] === 'object') {
            const firstQuestion = data.questions[0];
            if (firstQuestion.generated_by === 'gemini') {
              setGenerationSource('🤖 Generated by Gemini AI');
            } else {
              setGenerationSource('⚙️ Generated by LLM service');
            }
          } else {
            setGenerationSource('🤖 Generated by AI');
          }
  ...
  const fallbackGenerateQuestions = () => {
    ...
    setTimeout(() => {
      // Check if we have fallback questions for this topic
      const lowerPrompt = prompt.toLowerCase();
      let result = [];

      // Try to match with our fallback data
      if (lowerPrompt.includes('javascript') || lowerPrompt.includes('js')) {
        result = fallbackQuestions.javascript;
      } else if (lowerPrompt.includes('react')) {
        result = fallbackQuestions.react;
      } else if (lowerPrompt.includes('system') || lowerPrompt.includes('design')) {
        result = fallbackQuestions['system design'];
      } else {
      ...
      setGenerationSource('📝 Local fallback questions (AI service unavailable)');
      setLoading(false);
    }, 1500);
  };
